...
public class Armeria extends Componentes{
            ...
            for(int j  = 0; j< owner.tablero[i].length;j++){
                ...
                if(distanciaTmp1 <= 2 || distanciaTmp2 <= 2){
                    if(owner.tablero[i][j].explotado == 0 && owner.tablero[i][j].componente != null){
                       ...
                       res += "La casilla ("+j+","+i+") fue afectada debido al radio de explosion de una armeria ubicada en ("+x1+","+y1+") y en ("+x2+","+y2+") \n";
    ...
    public String morir(Player atacante) {
        return "El jugador "+atacante.nombre + " destruyo una estructura de "+tipoArma+" de "+ this.owner.nombre+"\n";
    }
